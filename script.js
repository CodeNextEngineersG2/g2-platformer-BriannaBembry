...
function buildLevel() {
  ...
  createMonster(500, 600, 0); //come back and change velocity later
  createMonster(300,600,0); 

  //My Level
  createPlatform(800,520,3); 
  createCollectable(1055,250); 
  createMonster(910,390,0); 
  createPlatform(1300,320,7); 
  createMonster(1595, 195, 0); 
  /*if(Monster.position.x >= 1279 || 2089){
    Monster.velocity*= -1; 
  }
  */
  createMonster(1860,195,0); 
  createCollectable(1665,45); 
  createPlatform(2300,690,2); 
  createMonster(2350,565,0); 
...
function applyGravity() {
  ...
  if(player.position.y >= height+27){
    executeLoss(); 
  }
...
// x velocity to 0.
function checkIdle() {
  if(!keyIsDown(LEFT_ARROW) || (RIGHT_ARROW) && playerGrounded == true){
...
function keyReleased() {
  /*How do I check if the player has released the up arrow key?
  if(player.velocity.y < 0){
    currentJumpTime = 0; 
  } */
}
